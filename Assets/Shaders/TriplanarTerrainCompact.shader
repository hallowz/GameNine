Shader "Voidborne/TriplanarTerrainCompact"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            struct Attributes
            {
                float4 positionOS : POSITION;  // Float16 x 4 auto-promoted to float4
                float4 normalOct  : NORMAL;    // SNorm8 x 4, xy = octahedral encoded, zw unused
                float4 color      : COLOR;     // UNorm8 x 4, texture group blend weights
                float4 uv2        : TEXCOORD2; // UNorm8 x 4, ore data: x=oreType, y=oreBlend, z=skyExposure
                float4 uv3        : TEXCOORD3; // UNorm8 x 4, per-vertex biome tint
            };
            ...
            Varyings vert(Attributes input)
            {
                ...
                float3 normalOS = OctDecode(input.normalOct.xy);
