...
namespace Voidborne.World.Chunks
{
    ...
    public class ChunkLoader : MonoBehaviour
    {
        ...
        private void Update()
        {
            ...
            // often at speed doesn't hurt loading quality — it just prevents wasting time
            // rebuilding queues that haven't drained yet.
            int adaptiveInterval = GetAdaptiveCheckInterval();
            ...
            unloadFrameCounter++;
            if (unloadFrameCounter >= adaptiveInterval && target != null && chunkManager != null)
            {
        ...
        /// <summary>
        /// Returns an adaptive check interval based on player speed.
        /// Walking speed: use base interval (2 frames).
        /// Vehicle speed: use longer interval (up to 6 frames) to prevent
        /// rebuilding queues before they've had a chance to drain.
        /// </summary>
        private int GetAdaptiveCheckInterval()
        {
            float speed = playerVelocity.magnitude;
            if (speed < 5f) return checkInterval;       // Walking
            if (speed < 15f) return checkInterval + 1;   // Running
            if (speed < 30f) return checkInterval + 2;   // Slow vehicle
            return checkInterval + 4;                     // Fast vehicle
        }

        /// <summary>
        /// Check which chunks need to be loaded or unloaded based on the target's position.
        /// At high speed, the loading center is shifted ahead in the direction of travel
        /// so chunks are generated before the player arrives, eliminating pop-in.
        /// </summary>
        private void CheckChunks()
        {
            ...
            // Velocity-shifted loading origin: at speed, shift the loading center
            // 1-2 chunks ahead so we generate chunks the player is about to enter.
            float speed = playerVelocity.magnitude;
            if (speed > 10f)
            {
                float chunkSize = ChunkData.SIZE;
                // Shift up to 2 chunks ahead
                float lookAheadChunks = Mathf.Clamp(speed * 0.1f / chunkSize, 0f, 2f);
                Vector3 velDir = playerVelocity / speed;
                playerPos += velDir * (lookAheadChunks * chunkSize);
            }
            ...
            // Rebuild LOD0 queue unless it's still mostly full from the last rebuild.
            // At vehicle speed, the player can cross chunk boundaries faster than the
            // queue drains, making rebuilds wasteful. Skip if >60% of entries remain.
            bool skipLod0Rebuild = lod0Queued && lod0Queue.Count > 0
                && speed > 10f
                && lod0Queue.Count > lod0Queue.TotalEnqueued * 0.6f;
