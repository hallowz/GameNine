...
| P3 | P3.1 liquid field + storage | ✓ 2026-06-11 — `World/Liquid/{LiquidTypes,LiquidField}.cs`; 1 byte/voxel, sparse (null until liquid enters, collapses when drained); typed liquids with viscosity table |
| P3 | P3.2 CA sim + activity set | ✓ 2026-06-11 — `World/Liquid/{LiquidSimJob,LiquidSimulator}.cs`; Burst per-chunk jobs, deterministic bottom-up fall→spread→settle; cross-chunk via tick-start border planes + outflow buffers with refund-on-overfill (conserving); sleep/wake set (settled = zero cost); MIN_SPREAD=8 floor added during testing — without it any open-edged pool eroded to nothing (edge cells halved into sub-visible dribbles that evaporated ring by ring); pressure/U-tube pass deferred to a P3 follow-up; 14 EditMode tests incl. conservation + determinism hash |
| P3 | P3.3–P3.6 (meshing, shader, sources, gameplay) | ☐ next — P3.3 needs MarchingCubesAdapter slot reservation + ChunkManager integration (register/unregister chunks with the simulator, Wake from TerrainDeformer, OnChunkLiquidChanged → remesh, liquid blob in P2.1 save format) |
...
- 2026-06-11 (P3.1+P3.2): compiles clean, 439/439 EditMode tests pass (14 new liquid tests). First run caught the pool edge-bleed flaw (fixed with MIN_SPREAD). Sim is standalone — no rendering yet, so nothing visible in-game until P3.3.
